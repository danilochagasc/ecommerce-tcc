package com.br.danilo.tcc.checkout.adapter.http.error
...
import com.br.danilo.tcc.checkout.core.domain.coupon.CouponExpiredException
...
class GlobalErrorHandler : WebExceptionHandler {
    ...
    override fun handle(
        ...
        val (status, message) =
            when (ex) {
                is CouponNotFoundException -> HttpStatus.NOT_FOUND to ex.message
                is CouponAlreadyExistsException -> HttpStatus.CONFLICT to ex.message
                is CouponExpiredException -> HttpStatus.BAD_REQUEST to ex.message
                is CartNotFoundException -> HttpStatus.NOT_FOUND to ex.message
