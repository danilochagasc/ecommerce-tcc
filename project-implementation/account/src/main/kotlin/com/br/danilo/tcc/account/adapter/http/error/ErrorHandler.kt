package com.br.danilo.tcc.account.adapter.http.error

import com.br.danilo.tcc.account.core.domain.address.AddressAlreadyExistsException
import com.br.danilo.tcc.account.core.domain.address.AddressNotBelongsToUserException
...
class GlobalErrorHandler : WebExceptionHandler {
    ...
    override fun handle(
        ...
        val (status, message) =
            when (ex) {
                is AddressNotFoundException -> HttpStatus.NOT_FOUND to ex.message
                is AddressAlreadyExistsException -> HttpStatus.CONFLICT to ex.message
                is AddressNotBelongsToUserException -> HttpStatus.BAD_REQUEST to ex.message
                is UserNotFoundException -> HttpStatus.NOT_FOUND to ex.message
                is UserNotFoundExceptionByEmail -> HttpStatus.NOT_FOUND to ex.message
                is UserAlreadyExistsException -> HttpStatus.CONFLICT to ex.message
                is PasswordsDoNotMatchException -> HttpStatus.BAD_REQUEST to ex.message
                is InvalidCredentialsException -> HttpStatus.UNAUTHORIZED to ex.message
